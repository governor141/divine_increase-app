...
/// After the Firestore write succeeds, this also does a best-effort POST
/// to the same Cloudflare Worker the website uses
/// (divine-increase-testimony.governoreze24.workers.dev) so it shows up
/// in Telegram with Approve/Reject buttons, exactly like a website
/// submission does. If that call fails (e.g. no signal), the testimony
/// is still saved and can be approved later from the website's "Pending
/// Testimonies" list — matching the website's own fallback behavior.
...
class _SubmitTestimonyScreenState extends State<SubmitTestimonyScreen> {
  static const _reviewWorkerUrl = 'https://divine-increase-testimony.governoreze24.workers.dev/';
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Same fallback the website uses: displayName, else the part of the
      // email before the @ sign — never "Anonymous" for a signed-in user.
      final authorName = (user?.displayName?.isNotEmpty == true)
          ? user!.displayName!
          : (email.contains('@') ? email.split('@')[0] : 'Anonymous');
      ...
      // Best-effort — matches the website's own error handling: if this
      // fails, the testimony is still saved and just waits in the
      // dashboard's "Pending Testimonies" list instead of Telegram.
      try {
        await http.post(
          Uri.parse(_reviewWorkerUrl),
        ...
        // Firestore save already succeeded; Telegram notification is optional.
      }
